...
- 노트북LM 과 구글 안티그래비티 (Google Antigravity) 를 이용해서 바이브 코딩으로 만든 2048 게임
- 총 제작 시간은 약 8시간.

- 2048 게임 로직과 게임 구현 오픈소스 코드는 노트북LM 을 통해서 분석을 해서 
  구글 안티그래비티 (Antigravity) 바이브 코딩용 프롬프트 를 생성
  (노트북LM 이 만들어준 프롬프트 는 아래에 추가)


기존 게임 구현은 안티그래비티 가 금방 만들어 주더군요.

게임을 몰입감, 효과를 위한 이미지 작업 및 추가 기능을 설정하는데 시간이 많이 걸렸습니다.
...
- \resource 폴더에 이미지를 바꾸시면 해당 회사 홍보용으로 사용 가능

- 빌딩, 비행기 이미지 작업은 Photopea 를 사용
